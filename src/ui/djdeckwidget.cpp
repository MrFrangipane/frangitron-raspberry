...
void DjDeckWidget::paint_(QRect rect)
{
    ...
    int selectedCue = int(_status.params[3].value) / DECK_ENCODER_REGION_RATIO; // TODO : WOWOWOWOWO !!!!
    ...
    if( selectedAudioFile >= 0 )
    {
        ...
        int regionWidth = infos.cues[selectedCue + 1].imagePosition - regionLeft;
...
QString DjDeckWidget::formatParameter(int paramId)
{
    if( paramId == 2) { // Track
        ...
        return QString::number(int(_status.params[3].value) / DECK_ENCODER_REGION_RATIO);
    }
